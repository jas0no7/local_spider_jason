...
class MyCrypto(object):
    ...
    @try_except(return_value='', is_log=True, logger_info='AES/ECB加密 {text}')
    def aes_ecb_encrypt(self, key: str or bytes, text: str, **kwargs) -> str:
        """
        AES ECB 加密
        :param key: 加密的key
        :param text: 加密的文本
        ...
        res = AES.new(key, AES.MODE_ECB).encrypt(pad(text.encode('utf-8'), AES.block_size, style='pkcs7'))
        return b64encode(res).decode('utf-8')

    @try_except(return_value='', is_log=True, logger_info='AES/ECB解密 {text}')
    def aes_ecb_decrypt(self, key: str or bytes, text: str, **kwargs) -> str:
        """
        AES ECB 解密
        :param key: 解密的key
        :param text: 加密的文本
        ...
        """
        key = key if isinstance(key, bytes) else key.encode('utf-8')
        result = AES.new(key, AES.MODE_ECB).decrypt(b64decode(text))
...
class MyMySQL(object):
    ...
    def __init__(self, connect: dict = None, db: str = None, maxconnections: int = 0):
        """
        :param connect: 连接参数
        :param db: 数据库名
        :param maxconnections: 连接最大数 0：不限
        """
        db = connect.get('db', db)
        if not db or not connect:
            raise Exception("请检查连接参数是否正确")
    ...
    @try_except(return_value=[{}], is_log=True, logger_info='查找数据')
    def select(self, sql, fetch=1, **kwargs):
        """
        查找数据
        :param sql: 查询语句
        :param fetch: 查询所有数据还是只查询一条 1：所有 0：一条
        :return:
        """
        connect, cursor = self.mysql_connect()
        cursor.execute(sql)
        if fetch == 1:
            result = cursor.fetchall()
        else:
            result = cursor.fetchone()
        self.mysql_close(connect, cursor)
        return result

    @try_except(return_value=False, is_log=True, logger_info='SQL执行语句')
    ...
    @try_except(return_value=None, is_log=True, logger_info='在表 {table_name} 中插入/更新数据 {item}')
    def insert_update(self, table_name: str, item: dict, unupdate: str = '', **kwargs):
        """
        在存在的时候更新数据
        :param table_name: 表名
        :param item: 保存的数据
        :param unupdate: 在插入更新的时候不用更新的字段
        :return:
        """
        unupdate = unupdate.split(',')
        connect, cursor = self.mysql_connect()
        columns = ', '.join([f"`{key}`" for key in item.keys()])
        placeholders = ', '.join(['%s'] * len(item))
        update_clause = ', '.join([f"`{column}`=VALUES(`{column}`)" for column in item.keys() if column not in unupdate])
        sql = f"INSERT INTO {table_name} ({columns}) VALUES ({placeholders}) ON DUPLICATE KEY UPDATE {update_clause}"
        values = [str(value) if (not isinstance(value, str)) and (value is not None) else value for value in item.values()]
        cursor.execute(sql, values)
        connect.commit()
        self.mysql_close(connect, cursor)

    @try_except(return_value=None, is_log=True, logger_info='在表 {table_name} 中添加分区 {partition_name}: {partition_value}')
    def add_partition(self, table_name, partition_name, partition_value):
        """
        添加表分区
        :param table_name: 表名
        :param partition_name: 分区名
        :param partition_value: 分区的值
        :return:
        """
        sql = f"""ALTER TABLE {table_name} ADD PARTITION (PARTITION `{partition_name}` VALUES LESS THAN ('{partition_value}'))"""
        connect, cursor = self.mysql_connect()
        cursor.execute(sql)
        connect.commit()
        self.mysql_close(connect, cursor)

    @try_except(return_value=[{}], is_log=False, logger_info='查询数据库 {database} 的表 {table_name} 的分区')
    def select_partition(self, database, table_name):
        """
        查询表分区
        :param database: 数据库
        :param table_name: 表名
        :return:
        """
        sql = f"""SELECT partition_name FROM information_schema.PARTITIONS WHERE TABLE_SCHEMA='{database}' AND TABLE_NAME='{table_name}' ORDER BY partition_description DESC"""
        connect, cursor = self.mysql_connect()
        cursor.execute(sql)
        result = cursor.fetchall()
        self.mysql_close(connect, cursor)
...
charge_topic = config.KAFKA_TOPIC
redis_key = config.REDIS_DUPLICATE
is_cq_key = config.REDIS_FILTER

YkcSichuanTemporary_producer = MyKafkaProducer()
myredis = MyRedis()
myredis.create(key=redis_key, capacity=50000000)
myredis.create(key=is_cq_key, capacity=20000)


def add_charge_partition(table_name, database=config.SPIDER_CHARGE_DATA, fmt="%Y%m%d"):
    """
    添加分区
    :param database: 数据库
    :param table_name: 表名
    ...
    :return:
    """
    start_date = get_now_date(fmt=None)
    end_date = start_date + timedelta(days=7)
    pars = mysql_charge.select_partition(database=database, table_name=table_name, is_log=False)
    pars = [par.get('partition_name') for par in pars]
    for date in date_range(start=start_date, end=end_date, freq="D"):
        partition_name = f"p{date.strftime(fmt)}"
        if partition_name in pars:
            continue
        mysql_charge.add_partition(table_name=table_name, partition_name=partition_name, partition_value=(date + timedelta(days=1)).strftime(fmt))


mysql_charge = MyMySQL(connect=config.MYSQL_211, db=config.SPIDER_CHARGE_DATA)
add_charge_partition(config.DB_DETAIL)
add_charge_partition(config.DB_TERMINAL)
...
def charge_filter(value):
    ...
    if myredis.exists(key=redis_key, value=value, is_log=False):
        return True
    myredis.add(key=redis_key, value=value, is_log=False)
    return False


def date_interval(step=30):
    """
    批次 ---> 2022-07-20 10:25:08 --> 属于 20220720100000 批次
    :param step: 每隔多久分一个区间
    :return: 转换后的批次
    """
    date_date = datetime.now(tz=ZoneInfo(key='PRC'))
    return date_date.replace(minute=(date_date.minute // step) * step).strftime('%Y%m%d%H%M00')


def YkcSichuanTemporary(item: dict):
    """
    充电站 统一处理数据格式 发送到kafka
    :param item: 需要保存的数据
    :return:
    """
    YkcSichuanTemporary_producer.send(topic=charge_topic, value=item)


def charge_filter_is_cq(value):
    """
    是否是重庆的站点
    :param value: 去重的字段值
    :return: True：不是重庆的, False：是重庆的
    """
    if myredis.exists(key=is_cq_key, value=value, is_log=False):
        return True
    return False
