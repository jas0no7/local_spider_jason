...
class DataSpider(CrawlSpider):
    ...
    infoes = [
        {
            'url': 'https://www.sc.gov.cn/10462/c103042/stt_list.shtml',
            'label': "首页;政府文件;最新文件",
            'detail_xpath': '//div[@class="mylieb"]/li',
            'url_xpath': './span[1]//a/@href',
            'title_xpath': './span[1]//a/text()',
            'publish_time_xpath': './span[2]//text()',
            'body_xpath': '//div[@class="articlebox"]',
            'total': 28,
            'page': 1,
            'base_url': 'https://www.sc.gov.cn/10462/c103042/stt_list_{}.shtml'
        },
        {
            'url': 'https://www.sc.gov.cn/10462/10464/13298/14097/bmjd_list.shtml',
            'label': "首页;政务公开;政策解读;部门解读",
            'detail_xpath': '//table[@id="dash-table"]//tr',
            'url_xpath': './td[2]//a/@href',
            'title_xpath': './td[2]//a/@title',
            'publish_time_xpath': '',
            'body_xpath': '//div[@id="articlecontent"]/div[@id="cmsArticleContent" or @class="zrbj"]',
            'total': 6,
            'page': 1,
            'base_url': 'https://www.sc.gov.cn/10462/10464/13298/14097/bmjd_list_{}.shtml'
        },
        {
            'url': 'https://www.sc.gov.cn/10462/10464/13298/13303/rdjd_list.shtml',
            'label': "首页;政务公开;政策解读;热点解读",
            'detail_xpath': '//table[@id="dash-table"]//tr',
            'url_xpath': './td[2]//a/@href',
            'title_xpath': './td[2]//a/@title',
            'publish_time_xpath': '',
            ...
            'total': 15,
            'page': 1,
            'base_url': 'https://www.sc.gov.cn/10462/10464/13298/13303/rdjd_list_{}.shtml'
        }
    ]
    ...
    def parse_detail(self, response):
        ...
        publish_time = _meta.get('publish_time') or \
                       ''.join(re.findall(r'发布日期.*?(\d{4}-\d{2}-\d{2})', response.text, re.DOTALL)).strip() or \
                       ''.join(response.xpath('//ul[@id="articleattribute"]/li[1]/text()').extract()).strip()
        ...
        author = response.xpath('//meta[@name="Author"]/@content').extract_first() or \
                 ...
                 ''.join(re.findall(r'>来源[:：](.*?)</', response.text, re.DOTALL)).strip() or \
