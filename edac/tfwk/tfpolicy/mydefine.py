...
def get_attachment(attachment_urls, url, spider_from):
    ...
    for att in attachment_urls:
        ...
        suffix_name_3 = Path(attachment_name).suffix
        if f"{suffix_url_1}".lower() not in attachment_suffix and suffix_url_2 not in attachment_suffix and '.pdf' not in attachment_url and suffix_name_3 not in attachment_suffix:
            continue
        ...
        if f"{suffix_url_1}".lower() in attachment_suffix:
            ...
            suffix = suffix_url_2
        elif suffix_name_3 in attachment_suffix:
            suffix = suffix_name_3
...
class UserAgent(object):
    ...
    def firefox_browser_version(self) -> dict:
        ...
        return {
            ...
            'geckotrail': datetime.fromtimestamp(randint(1577808000, int(datetime.now(tz=ZoneInfo(key='PRC')).timestamp()))).strftime('%Y%m%d')
        }
