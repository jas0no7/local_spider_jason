import requests
import json
import hashlib
import time
from bs4 import BeautifulSoup
import pandas as pd

timestamp = str(int(time.time() * 1000))
headers = {
    "Accept": "application/json, text/plain, */*",
    "Accept-Language": "zh-CN,zh;q=0.9",
    "Cache-Control": "no-cache",
    "Connection": "keep-alive",
    "Content-Type": "application/json",
    "Origin": "https://mapi.zjzwfw.gov.cn",
    "Pragma": "no-cache",
    "Referer": "https://mapi.zjzwfw.gov.cn/web/mgop/gov-open/zj/2001941911/reserved/index.html?",
    "User-Agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/141.0.0.0 Safari/537.36",
    "header_optimus_auth_request_token": "01fdaf34fd474896bab0a1aedddd835c"
}

cookies = {
    "$aliyungf_tc": "9d26ecfa85a2e2276e857eb4660522137183c44373b2e9bb962fd9981695588f",
    "webId": "1",
    "zjzw_siteCode": "330000000000",
    "arialoadData": "false",
    "JSESSIONID": "kKwotoHmZikLdzo7_2SEX2qK_WSRE_CK_Nd5jqpBd_myD6K-CISh!-1473847150",
    "cna": "/B+HIV6hMV4CAf////+DgJV0",
    "SERVERID": "fb4ec6a2aa4cbb746e0f0e492b2168b9|1761620816|1761619632"
}
categories = {
    "年度": "N",
    "季度": "J",
    "月度": "Y"
}
n_categories = {
    "交通邮电": "1100",
    "人口": "0200",
    "价格": "1200",
    "其他社会活动": "2600",
    "农业": "1400",
    "固定资产投资和房地产": "1800",
    "国内贸易": "2000",
    "国民经济核算": "1300",
    "城市建设": "0500",
    "对外经济": "2100",
    "就业与工资": "0300",
    "居民住户": "2700",
    "居民收支": "1900",
    "工业": "1500",
    "建筑业": "1700",
    "教育": "2300",
    "数字化转型": "540617cec8d84ae7b09019d86dbc08a0",
    "文化体育卫生": "2500",
    "旅游": "2200",
    "服务业": "0400",
    "环境保护": "0600",
    "科技": "2400",
    "能源": "1600",
    "行政区划和自然资源": "0100",
    "财政": "0900",
    "金融": "1000"
}

j_categories = {
    "价格": "1200",
    "农业": "1400",
    "国民经济核算": "1300",
    "居民收支": "1900",
    "数字化转型": "540617cec8d84ae7b09019d86dbc08a0"
}

y_categories = {
    "价格": "1200",
    "固定资产投资和房地产": "1800",
    "国际贸易": "2000",
    "工业": "1500",
    "数字化转型": "540617cec8d84ae7b09019d86dbc08a0",
    "服务业": "0400",
    "能源": "1600"
}


def generate_sign(token, ak, api, ts, data):
    # 按照指定格式拼接字符串
    sign_str = f"token={token}&ak={ak}&api={api}&ts={ts}&data={data}"

    # 计算MD5哈希值
    md5_hash = hashlib.md5(sign_str.encode('utf-8')).hexdigest()

    # 转换为小写
    return md5_hash.lower()


url = "https://mapi.zjzwfw.gov.cn/h5/mgop"
params = {
    "token": '',
    "ak": "udqmn52d+2001941911+elgttz",

    "api": "mgop.gw.sjzj.zbcxqueryInstrumentHn",
    "ts": timestamp,

    "data": "null"
}
signature = generate_sign(**params)
params['sign'] = signature
# for i in range(1, 710):  # 页码从1到710
post_data = {
    "data": [
        {"vtype": "pagination", "name": "pagerows", "data": 10},
        {"vtype": "pagination", "name": "totalrows", "data": 0},
        {"vtype": "pagination", "name": "page", "data": 1},
        {"vtype": "pagination", "name": "sortName", "data": ""},
        {"vtype": "pagination", "name": "sortFlag", "data": ""}
    ]
}
for bgqb_name, bgqb_data in categories.items():
    if bgqb_name == "年度":

        for group_name, group_urn in n_categories.items():

            data = {
                "postData": json.dumps(post_data, separators=(',', ':')),
                "bh": "",
                "ddiInstance": "urn:ddi:ZJJCKSTAT:a463a4d9-806b-4fcc-8fc9-b43a8ee4298c:1",
                "groupUrn": group_urn,
                "studyUnitUrn": "",
                "instrumentSchemeUrn": "",
                "bgqb": "N",
                "projectId": "zj",
                "dqCode": "33"
            }

            # 转换为 JSON 字符串
            data = json.dumps(data, separators=(',', ':'))

            # 发送请求
            response = requests.post(url, headers=headers, cookies=cookies, params=params, data=data)

            response.encoding = 'utf-8'

            # 解析返回的 JSON
            data_json = json.loads(response.text)
            rows = data_json["data"]["data"][0]["data"]["rows"]

            for i, item in enumerate(rows):
                bsItemUrn = data_json["data"]["data"][0]["data"]["rows"][i]["bsItemUrn"]
                bgq = data_json["data"]["data"][0]["data"]["rows"][i]["bgq"]
                taskId = data_json["data"]["data"][0]["data"]["rows"][i]["taskId"]
                instrumentNumber = data_json["data"]["data"][0]["data"]["rows"][i]["instrumentNumber"]
                urls = headers["Referer"] + f"#/wholeTableData/queryDetail?taskId={taskId}&bgq={bgq}&instrumentNumber={instrumentNumber}"
                id = data_json["data"]["data"][0]["data"]["rows"][i]["Id"]

                post_data_detail = {
                    "data": [
                        {"vtype": "attr", "name": "bsItemUrn", "data": bsItemUrn},
                        {"vtype": "attr", "name": "bgq", "data": bgq},
                        {"vtype": "attr", "name": "dqCode", "data": "33"},
                        {"vtype": "attr", "name": "instrumentNumber", "data": instrumentNumber}
                    ]
                }
                data_detail = {
                    "postData": json.dumps(post_data_detail, separators=(',', ':')),
                }
                data_detail = json.dumps(data_detail, separators=(',', ':'))
                params_detail = {
                    "token": "",
                    "ak": "udqmn52d+2001941911+elgttz",
                    "api": "mgop.gw.sjzj.zbcxgetReportHtmlByCondition",
                    "ts": timestamp,
                    "data": "null"
                }
                sign_detail = generate_sign(**params_detail)
                params_detail['sign'] = sign_detail
                response_detail = requests.post(url, headers=headers, cookies=cookies, params=params_detail, data=data_detail)

                data_json_detail = json.loads(response_detail.text)

                html_raw = data_json_detail["data"]["data"][4]["data"]  # 这里取出完整的 HTML 字符串
                soup = BeautifulSoup(html_raw, "html.parser")

                # 2. 提取表格（只取第一个表格）
                table_html = str(soup.find("table"))
                #
                # 3. 使用 pandas 解析为 DataFrame（无需保存 Excel）
                content = pd.read_html(table_html)[0]

                publish_time = data_json_detail["data"]["data"][0]["data"]
                title = data_json_detail["data"]["data"][1]["data"]
                body_html = html_raw
                content = content
                images = [],
                label = f"{bgqb_name};{group_name}"
                print({
                    "id": id,
                    "url": urls,
                    "label": label,
                    "body_html": body_html,
                    "publish_time": publish_time,
                    "title": title,
                    "content": content.to_dict() if isinstance(content, pd.DataFrame) else content,
                    "images": images,
                    "report_period": bgq,
                })
                # with open("result1.json", "a", encoding="utf-8") as f:
                #     json.dump({
                #         "id": id,
                #         "url": urls,
                #         "label": label,
                #         "body_html": body_html,
                #         "publish_time": publish_time,
                #         "title": title,
                #         "content": content.to_dict() if isinstance(content, pd.DataFrame) else content,
                #         "images": images,
                #         "report_period": bgq,
                #     }, f, ensure_ascii=False)
                #     f.write("\n")

    elif bgqb_name == "季度":

        for group_name, group_urn in j_categories.items():
            data = {
                "postData": json.dumps(post_data, separators=(',', ':')),
                "bh": "",
                "ddiInstance": "urn:ddi:ZJJCKSTAT:a463a4d9-806b-4fcc-8fc9-b43a8ee4298c:1",
                "groupUrn": group_urn,
                "studyUnitUrn": "",
                "instrumentSchemeUrn": "",
                "bgqb": bgqb_data,
                "projectId": "zj",
                "dqCode": "33"
            }

            # 转换为 JSON 字符串
            data = json.dumps(data, separators=(',', ':'))

            # 发送请求
            response = requests.post(url, headers=headers, cookies=cookies, params=params, data=data)

            response.encoding = 'utf-8'

            # 解析返回的 JSON
            data_json = json.loads(response.text)

            rows = data_json["data"]["data"][0]["data"]["rows"]

            for i, item in enumerate(rows):
                bsItemUrn = data_json["data"]["data"][0]["data"]["rows"][i]["bsItemUrn"]
                bgq = data_json["data"]["data"][0]["data"]["rows"][i]["bgq"]
                taskId = data_json["data"]["data"][0]["data"]["rows"][i]["taskId"]
                instrumentNumber = data_json["data"]["data"][0]["data"]["rows"][i]["instrumentNumber"]
                urls = headers["Referer"] + f"#/wholeTableData/queryDetail?taskId={taskId}&bgq={bgq}&instrumentNumber={instrumentNumber}"
                id = data_json["data"]["data"][0]["data"]["rows"][i]["Id"]

                post_data_detail = {
                    "data": [
                        {"vtype": "attr", "name": "bsItemUrn", "data": bsItemUrn},
                        {"vtype": "attr", "name": "bgq", "data": bgq},
                        {"vtype": "attr", "name": "dqCode", "data": "33"},
                        {"vtype": "attr", "name": "instrumentNumber", "data": instrumentNumber}
                    ]
                }
                data_detail = {
                    "postData": json.dumps(post_data_detail, separators=(',', ':')),
                }
                data_detail = json.dumps(data_detail, separators=(',', ':'))
                params_detail = {
                    "token": "",
                    "ak": "udqmn52d+2001941911+elgttz",
                    "api": "mgop.gw.sjzj.zbcxgetReportHtmlByCondition",
                    "ts": timestamp,
                    "data": "null"
                }
                sign_detail = generate_sign(**params_detail)
                params_detail['sign'] = sign_detail
                response_detail = requests.post(url, headers=headers, cookies=cookies, params=params_detail, data=data_detail)

                data_json_detail = json.loads(response_detail.text)

                html_raw = data_json_detail["data"]["data"][4]["data"]  # 这里取出完整的 HTML 字符串
                soup = BeautifulSoup(html_raw, "html.parser")

                # 2. 提取表格（只取第一个表格）
                table_html = str(soup.find("table"))
                #
                # 3. 使用 pandas 解析为 DataFrame（无需保存 Excel）
                content = pd.read_html(table_html)[0]

                publish_time = data_json_detail["data"]["data"][0]["data"]
                title = data_json_detail["data"]["data"][1]["data"]
                body_html = html_raw
                content = content
                images = [],
                label = f"{bgqb_name};{group_name}"
                print({
                    "id": id,
                    "url": urls,
                    "label": label,
                    "body_html": body_html,
                    "publish_time": publish_time,
                    "title": title,
                    "content": content.to_dict() if isinstance(content, pd.DataFrame) else content,
                    "images": images,
                    "report_period": bgq,
                })
                # with open("result1.json", "a", encoding="utf-8") as f:
                #     json.dump({
                #         "id": id,
                #         "url": urls,
                #         "label": label,
                #         "body_html": body_html,
                #         "publish_time": publish_time,
                #         "title": title,
                #         "content": content.to_dict() if isinstance(content, pd.DataFrame) else content,
                #         "images": images,
                #         "report_period": bgq
                #     }, f, ensure_ascii=False)
                #     f.write("\n")


    else:
        for group_name, group_urn in y_categories.items():
            data = {
                "postData": json.dumps(post_data, separators=(',', ':')),
                "bh": "",
                "ddiInstance": "urn:ddi:ZJJCKSTAT:a463a4d9-806b-4fcc-8fc9-b43a8ee4298c:1",
                "groupUrn": group_urn,
                "studyUnitUrn": "",
                "instrumentSchemeUrn": "",
                "bgqb": bgqb_data,
                "projectId": "zj",
                "dqCode": "33"
            }

            # 转换为 JSON 字符串
            data = json.dumps(data, separators=(',', ':'))

            # 发送请求
            response = requests.post(url, headers=headers, cookies=cookies, params=params, data=data)

            response.encoding = 'utf-8'

            # 解析返回的 JSON
            data_json = json.loads(response.text)
            rows = data_json["data"]["data"][0]["data"]["rows"]

            for i, item in enumerate(rows):
                bsItemUrn = data_json["data"]["data"][0]["data"]["rows"][i]["bsItemUrn"]
                bgq = data_json["data"]["data"][0]["data"]["rows"][i]["bgq"]
                taskId = data_json["data"]["data"][0]["data"]["rows"][i]["taskId"]
                instrumentNumber = data_json["data"]["data"][0]["data"]["rows"][i]["instrumentNumber"]
                urls = headers["Referer"] + f"#/wholeTableData/queryDetail?taskId={taskId}&bgq={bgq}&instrumentNumber={instrumentNumber}"
                id = data_json["data"]["data"][0]["data"]["rows"][i]["Id"]

                post_data_detail = {
                    "data": [
                        {"vtype": "attr", "name": "bsItemUrn", "data": bsItemUrn},
                        {"vtype": "attr", "name": "bgq", "data": bgq},
                        {"vtype": "attr", "name": "dqCode", "data": "33"},
                        {"vtype": "attr", "name": "instrumentNumber", "data": instrumentNumber}
                    ]
                }
                data_detail = {
                    "postData": json.dumps(post_data_detail, separators=(',', ':')),
                }
                data_detail = json.dumps(data_detail, separators=(',', ':'))
                params_detail = {
                    "token": "",
                    "ak": "udqmn52d+2001941911+elgttz",
                    "api": "mgop.gw.sjzj.zbcxgetReportHtmlByCondition",
                    "ts": timestamp,
                    "data": "null"
                }
                sign_detail = generate_sign(**params_detail)
                params_detail['sign'] = sign_detail
                response_detail = requests.post(url, headers=headers, cookies=cookies, params=params_detail, data=data_detail)

                data_json_detail = json.loads(response_detail.text)

                html_raw = data_json_detail["data"]["data"][4]["data"]  # 这里取出完整的 HTML 字符串
                soup = BeautifulSoup(html_raw, "html.parser")

                # 2. 提取表格（只取第一个表格）
                table_html = str(soup.find("table"))
                #
                # 3. 使用 pandas 解析为 DataFrame（无需保存 Excel）
                content = pd.read_html(table_html)[0]

                publish_time = data_json_detail["data"]["data"][0]["data"]
                title = data_json_detail["data"]["data"][1]["data"]
                body_html = html_raw
                content = content
                images = [],
                label = f"{bgqb_name};{group_name}"
                print({
                    "id": id,
                    "url": urls,
                    "label": label,
                    "body_html": body_html,
                    "publish_time": publish_time,
                    "title": title,
                    "content": content.to_dict() if isinstance(content, pd.DataFrame) else content,
                    "images": images,
                    "report_period": bgq,
                })
                # with open("result1.json", "a", encoding="utf-8") as f:
                #     json.dump({
                #         "id": id,
                #         "url": urls,
                #         "label": label,
                #         "body_html": body_html,
                #         "publish_time": publish_time,
                #         "title": title,
                #         "content": content.to_dict() if isinstance(content, pd.DataFrame) else content,
                #         "images": images,
                #         "report_period": bgq,
                #     }, f, ensure_ascii=False)
                #     f.write("\n")
