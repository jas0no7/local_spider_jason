...
base_headers = {
    ...
    "cookie": "u=2; JSESSIONID=tze5DGw4CxSVC8hFFY_odDb42dUh6T0jcfJ01awKT-9ElSkYE8Kh!-993230399"
}
...
# 加载输入文件
# ==========================================================
with open("zhibiao_list.json", "r", encoding="utf-8") as f:
    zhibiao = json.load(f)
        ...
        for le2_dict in le2_list:
            for le2_name, leaf_list in le2_dict.items():
                ...
                for item in leaf_list:
                    ...
                    # ==========================================================
                    # 自动适配表名（table_name）与 indicator_id
                    # 若存在 le3 → 使用 le3
                    # 若不存在（你的情况）→ 使用 le2
                    # ==========================================================
                    indicator_id = None
                    ...
                    # 情况 1：存在三级指标
                    if isinstance(item, dict) and "le3_id" in item and "le3_name" in item:
                        indicator_id = item["le3_id"]
                        indicator_name = item["le3_name"]

                    # 情况 2：无三级指标（你的 JSON 属于此情况）
                    elif isinstance(item, dict) and "le2_id" in item and "le2_name" in item:
                        indicator_id = item["le2_id"]
                        indicator_name = item["le2_name"]

                    else:
                        logger.warning(f"未知的指标结构: {item}")
                        continue
